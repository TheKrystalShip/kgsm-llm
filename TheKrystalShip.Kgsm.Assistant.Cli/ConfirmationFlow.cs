...
internal static class ConfirmationFlow
{
    ...
    private static string Describe(PendingConfirmation c) => c.Kind switch
    {
        ...
        // The content itself isn't printed (it can be sizeable) — just which file changes and how big
        // it ends up; the CLI's y/N is the confirmation, not a diff viewer (owed to a later surface).
        ConfirmationKind.WriteFile =>
            $"edit '{c.ConfigKey}' on '{c.Target}' ({(c.ConfigValue?.Length ?? 0):N0} chars after the edit)",
