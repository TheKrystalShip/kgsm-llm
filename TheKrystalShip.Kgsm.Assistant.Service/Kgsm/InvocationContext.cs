...
/// (<see cref="Actor"/>, the verified Discord principal, e.g. <c>discord:Haru</c>) and <em>through
/// which surface</em> (<see cref="Origin"/>, always <c>assistant</c> for this service). Flows to KGSM as
...
{
    /// <summary>This service's surface tag (architecture.html §3·d origin set).</summary>
...
/// <summary>
/// Ambient holder of the current <see cref="Invocation"/>, set per request at an HTTP entry point
/// (<c>/turn</c>, <c>/confirm</c>) from the authenticated principal and read at the kgsm chokepoint
/// (<c>KgsmServerOperations</c>) so every mutating call stamps provenance — without threading an actor
/// through the assistant agent loop or the shared <c>IServerOperations</c> port.
/// </summary>
/// <remarks>
