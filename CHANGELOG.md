...
certificate for a name it holds and has published, the member requests a new one straight away
instead of waiting for the certificate's own renewal window.
...
administered as one of its services. On a machine in a cluster it is an **anchor** — a member in its
own right, claiming the `assistant` capability, reached at an address rather than through a
neighbour, and answering for the whole cluster rather than for whichever machine it happens to run
on.

Which one it is depends on the deployment, not on the build. `/etc/kgsm/kgsm-cluster.env` holding a
secret is the whole of the switch, and it is the same file the rest of the ecosystem's members read —
so the running service and the descriptor beside it can never disagree about what this component is.
The build writes both descriptors and the deploy installs the one this host's standing calls for,
clearing the other; a machine that joins a cluster changes what it is by re-running `deploy.sh`.

One assistant per cluster, for the same reason there is one member holding the accounts: a person asks
a question and there is one answer to where it goes. A second install claims only into an assignment
nobody holds, re-reads, and stands down when the cluster names somebody else.

`Cluster:MemberId` names this member — per member and never per host, since a machine running a node
as well holds two members with two names. `Cluster:PublicBaseUrl` is the address a browser reaches the
chat at, stated rather than observed, because an assistant on a LAN address behind a public vhost has
two answers and handing a browser the first sends it somewhere it cannot reach.
...
Every event the assistant writes carries an `Id`: a UUIDv7 the shared writer mints per line, inherited by pinning
...
to route: the turn simply never looked.

**Answering from memory something you were asked to look up is a fabricated status**, the ecosystem
rule applied to a fact about the world rather than about a server. The reply reads as researched, is
not, and says nothing about the difference.

The turn now records whether a search actually ran, and the reply review — the same mechanism that
catches a reply claiming an action the turn never took — re-prompts once when the person asked for the
web and nothing was looked up. The nudge restates the request, because the instruction is routinely a
follow-up carrying no subject: *"look it up online"* means the thing just discussed, and a nudge that
does not say so invites a search for those four words. If the second attempt still does not look, the
reply stands with an honest note that it was not looked up, rather than being discarded — the content
may still be useful, and the person is waiting.

A search that runs and finds nothing **counts as having looked**: "I searched and found nothing" is a
real answer to "check online", and only never looking is the failure.
...
It builds two packages: the assistant Service and CLI, and `kgsm-rag-indexer` for the retrieval
index. The CLI lands on PATH at `/usr/bin/kgsm-assistant-cli`.
